				...
				.click(
						function() {
							$.ajax({
										...
										error : function(err) {
											alert("系统错误，请联系系统管理员1！"+$("input[name='serverurl']")
													.val());
										}
									});
						});
...
//			document.getElementById("yjcontent").value = "驳回";
	...
	function getId(state) {
		$.ajax({
			...
			error : function(err) {
				alert("系统错误，请联系系统管理员！2"+$("input[name='serverurl']")
						.val());
			}
		});
	}
	...
	function submit(state) {
		$.ajax({
			...
			error : function(err) {
				alert("系统错误，请联系系统管理员！4"+$("input[name='serverurl']")
						.val());
			}
		});
	}
...
<body>
	<div class="a2">
			...
			<table style="background-color:white;width:100%;margin-top:15px;">
				<tr style="font-size:15px;color:#999; height:36px;width:100%;">
					...
					<div style="float:left;"><!-- --制单人:${data.smake} ${dateks.name}的${dateks.title} -->
						<span style="font-size: 12px">&nbsp;&nbsp;提交人:${dateks.name}</span><br/>&nbsp;&nbsp;<span style="color:#8FB95C;font-size: 11px">
						<c:if test="${_right==1}">
						已完成
		 				</c:if>
		 				<c:if test="${_right==2}">
		...
		<p style="color:#999;font-size:15px;">
			...
			<c:if test="${data.documentstype eq 'C10901' || data.documentstype eq 'C10309' || data.doucmentstype eq 'C10912'}">
			...
			<c:if test="${data.state !=0}">
				<tr class="tr">
					<td>审批意见:<textarea disabled="disabled" style="width: 100%;"
							name="yjcontent" rows="5" placeholder="审批意见:">${data.yjcontent eq 'null'?"":data.yjcontent}</textarea></td>
				</tr>
				<tr class="tr">
