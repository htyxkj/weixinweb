...
//		String Secret=jsonObject.getString("w_secret");//管理组标识
		...
		for (int i = 0; i < arry.size(); i++) {
			...
			if (null != accessToken) {
	        	...
	        	TokenThread.maplist.put(CorpIDid+"-"+json.getString("w_applyid"), accessToken);
			}
			//获取微信应用的消息
	...
	public String updateAppUrl(String wxscmid,String appid,String bipAppid,String redirect_domain) throws UnsupportedEncodingException{
		...
		AccessToken acc = (AccessToken) map.get(wxscmid+"-"+appid);
		WeixinUtil wu=new WeixinUtil();
		//应用指向的url
		...
		if(bipAppid.equals("01")){
			url=""+redirect_domain+"/weixinweb/ButtonServlet?w_appid="+appid+"&state=0&wxscmid="+wxscmid;
		}
		//02任务中心
		...
		if(bipAppid.equals("04")){
			url=""+redirect_domain+"/weixinweb/OaggtzServlet?w_appid="+appid+"&read=0&wxscmid="+wxscmid;
		}
		//报销中心
